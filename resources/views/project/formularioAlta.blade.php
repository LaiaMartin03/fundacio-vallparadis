<x-app-layout>
    ...
    <div class="py-12 max-w-7xl mx-auto sm:px-6 lg:px-8 bg-white rounded-xl">
        <h1 class="text-2xl font-bold mb-4">Nou projecte</h1>
        ...
        <form action="{{ route('project.store') }}" method="POST" class="space-y-4">
            ...
            <x-text-input id="description" name="description" type="text" placeholder="Descripció del Projecte" class="mt-1 block w-full" :value="old('description')" />
            <x-input-error :messages="$errors->get('description')" class="mt-2" />

            <x-text-input id="observations" name="observations" type="text" placeholder="Observacions del Projecte" class="mt-1 block w-full" :value="old('observations')" />
            <x-input-error :messages="$errors->get('observations')" class="mt-2" />

            <div class="flex items-center gap-6 mt-1">
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="radio" name="type" value="project" class="text-blue-600 focus:ring-blue-500" checked
